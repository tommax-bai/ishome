...
> 团队假设：8-15人研发，Java业务班底 + Python AI管线班底（此假设变化需重算语言栈决策）
...
| # | 决策 | 变量 | 影响 |
|---|---|---|---|
| ① | 团队画像确认 | 是否为8-15人、Java+Python双班底；若5人以内起步则语言收敛回单栈 | 优先级高于一切架构图 |
| ② | Postgres vs MySQL | 班底熟悉度 vs PostGIS/JSONB表达力 | 存储层 |
| ③ | ~~审核工作台第一版范围~~ → 模板验收台范围（修订二缩编） | 是否含"换三款灯"候选管理与单品意图数据回流 | genpipe↔shelf契约复杂度 |
| ④ | 网关：云托管 vs 自托管APISIX | 对云厂商锁定的立场（供应商策略判断） | 网关层实现 |
