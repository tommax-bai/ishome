...
| 手册 2 判据 | schema 字段 + cr- 机检 + 判官反例库 | 机检 27 条已编译；**反例库来源 2026-08-28 改判**：不用 13 轮附录 A，改由冷启动真跑样本自建（规则 4.17 原文即"种子集初版来自冷启动期问题样本"，且真跑样本真实度高于旧文档转写；反例样例挂 cr- 判据之下，首批一律观察态） |
...
- 判官反例库：**由冷启动真跑样本编译为 release 数据**（2026-08-28 改判，原"13 轮附录 A 转化"作废）；样例挂 cr- 判据之下（承规则 4.10b：正当性锚仍是 decided_by，不新建第三类命名空间），入册过规则 4.17 三道门，首批一律观察态。**只收真跑观察到的样本，禁止想象填充**。
- pt- 页型库首版声明。
